#!/bin/bash
# Script d'entrypoint pour l'agent Wazuh sur Debian
...
# Supporte <server><address> et <manager><address>
if [ -f "$OSSEC_CONF" ]; then
    sed -i "s|<address>.*</address>|<address>${WAZUH_MANAGER}</address>|g" "$OSSEC_CONF"
fi

# Démarrer l'agent en arrière-plan si on reçoit des arguments
